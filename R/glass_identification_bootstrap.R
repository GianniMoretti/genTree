...
set.seed(1090)
# Albero evtree con profondità massima 5 e alpha = 0.25
#ev <- evtree(Type ~ ., data = Glass, minbucket = 10, maxdepth = 5, alpha = 0.25)
ev <- evtree(Type ~ ., data = Glass, minbucket = 10, maxdepth = 5)

# Visualizza e salva l'albero evtree
png("graph_glass/glass_ev.png", width = 1200, height = 600)
plot(ev)
dev.off()
...
bootstrap_oob_accuracy <- function(data, model_fun, n_iter = 100, ...) {
  ...
  for (i in seq_len(n_iter)) {
    ...
    train <- data[idx, ]
    test <- data[oob_idx, ]
    # Se non ci sono dati OOB, non si può calcolare l'accuracy/complessità
    if (nrow(test) == 0) {
      acc[i] <- NA
      comp[i] <- NA
      next
...
res_ct_boot <- bootstrap_oob_accuracy(Glass, fit_ctree)
res_ev_boot <- bootstrap_oob_accuracy(Glass, fit_evtree)
...
png("graph_glass/glass_accuracy_boxplot.png", width = 900, height = 600)
...
png("graph_glass/glass_complexity_boxplot.png", width = 900, height = 600)
